            ...
            <div class="row">
              <div class="col-lg-12">
                <div class="block margin-bottom-sm">
                  <?php 
                  ?>
                  <div class="title"><strong>Expence Table</strong></div>
                  <div class="table-responsive"> 
                    <table class="table">
                      ...
                      <tbody>
                      <?php 
                          $sqlp = "SELECT * FROM expence ORDER BY expenceid DESC";
                          $result1p = mysqli_query($connection, $sqlp);
                          $indexaa = 1;
                          $def = 10;
                          if (mysqli_num_rows($result1p) > 0) {
                          // output data of each row
                          while($expence = mysqli_fetch_assoc($result1p)) {
                              echo'
                              <tr>
                              <td>'.$index.'</td>
                              <td>'.$expence['fname'].'</td>
                              ...
                              <button type="button" data-toggle="modal'.$def.'" data-target="#myModal'.$def.'" class="btn btn-primary mt-4">ADD Expence</button>

                                            <div id="myModal'.$def.'" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel1" aria-hidden="true" class="modal fade text-left">
                                            <div role="document" class="modal-dialog">
                                              <div class="modal-content">
                                                <div class="modal-header"><strong id="exampleModalLabel" class="modal-title">Expence</strong>
                                                  <button type="button" data-dismiss="modal'.$def.'" aria-label="Close" class="close"><span aria-hidden="true">×</span></button>
                                                </div>
                          ...
                          if(isset($_GET['user'])){
                              ...
                              echo '<div class="alert alert-danger" role="alert">User was deleted successfully</div>';
                          }elseif(isset($_GET['updated'])){
                              echo '<div class="alert alert-info" role="alert">All changes are saved successfully</div>';
                          }
                      ?>
                              </tbody>
                    </table>
